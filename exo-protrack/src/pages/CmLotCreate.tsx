...
export default function CmLotCreate() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.raw_container_code || formData.nominal_volume_ml <= 0) {
      showError('Выберите посуду для сырья');
      return;
    }
      ...
      <Card>
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1">Режим производства</label>
              <select
                value={formData.mode}
            ...
            {formData.mode === 'MTO' && !mtoLineId && mtoRequests.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Выберите заявку (FEFO)</label>
                  ...
                  {mtoRequests.map(req => (
                    ...
                    >
                      ...
                      <div className="text-right text-sm">
                        {req.is_overdue && <Badge variant="destructive" className="mr-2">ПРОСРОЧЕНО</Badge>}
                        {req.is_urgent && <Badge variant="warning" className="mr-2">СРОЧНО</Badge>}
                        {req.due_date && (
                          <span className={`${req.is_overdue ? 'text-red-600' : req.is_urgent ? 'text-amber-600' : 'text-muted-foreground'}`}>
            ...
            {(formData.mode === 'MTS' || mtoLineId) && (
              <div>
                <label className="block text-sm font-medium text-muted-foreground mb-1">Продукт (сырье) *</label>
                <select
            ...
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1">Примечания</label>
              <textarea
