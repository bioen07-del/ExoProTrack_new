...
export default function RequestList() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (!formData.due_date) {
      showError('Срок исполнения обязателен!');
  ...
  async function handleDeleteRequest(requestId: string) {
    const ok = await showConfirm(`Удалить заявку ${requestId}?`);
    if (!ok) return;
    try {
    ...
    <div className="space-y-6">
      ...
      {showForm && !showPreview && (
        <Card>
          <CardContent className="p-6 space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-semibold text-foreground">Новая заявка</h3>
              <Button variant="ghost" size="icon-sm" onClick={() => { setShowForm(false); resetForm(); setSearchParams({}); }}>
                <X size={20} />
              </Button>
            </div>
            ...
            {/* Product Selection */}
            <div>
              <label className="block text-sm font-medium mb-1 text-foreground">Продукт *</label>
              <select
                      ...
                      <div className="text-xs space-y-1">
                        {(() => {
                          ...
                          return (
                            <>
                              <div>
                                <span className="text-muted-foreground">Сырьё:</span>{' '}
              ...
              <>
                <div className={`p-4 rounded-lg border ${formData.reserved_cm_lot_id ? 'bg-muted border-border opacity-60' : 'bg-blue-50 border-blue-200'}`}>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    {!formData.reserved_cm_lot_id && (
                      <div className="flex gap-2">
                        <Button type="button" size="sm" onClick={() => setShowQcRawModal(true)}>
                          + QC
                        </Button>
                        <Button type="button" size="sm" className="bg-purple-600 hover:bg-purple-700" onClick={() => setShowProcessRawModal(true)}>
                          + Процессинг
                        </Button>
                      </div>
                    )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {formData.reserved_cm_lot_id ? (
                      <span className="text-muted-foreground text-sm italic">Сырьё со склада — QC и процессинг уже пройдены</span>
                    ) : (
                      <>
                        ...
                        {formData.extra_raw_qc.length === 0 && formData.extra_raw_process.length === 0 && (
                          <span className="text-muted-foreground text-sm">Нет дополнительных</span>
                        )}
                      </>
                    )}
                  </div>
                ...
                <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button type="button" size="sm" className="bg-purple-600 hover:bg-purple-700" onClick={() => setShowProcessProductModal(true)}>
                        + Процессинг
                      </Button>
                      <Button type="button" size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={() => setShowPostProcessProductModal(true)}>
                        + Постпроцессинг
                      </Button>
                    </div>
                  </div>
            ...
            {productSpec && !(productSpec as any).default_pack_format_code && (
              <div>
                <label className="block text-sm font-medium mb-1 text-foreground">Формат упаковки (для продукта) *</label>
                <select
            ...
            {productSpec && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-foreground">Ссылка заказчика</label>
                  <Input
                    type="text"
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => { setShowForm(false); resetForm(); setSearchParams({}); }}>
                Отмена
              </Button>
              <Button
                type="button"
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Превью заявки</DialogTitle>
          </DialogHeader>
          ...
          <div className="p-4 bg-muted rounded-lg space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <Badge variant={formData.reserved_cm_lot_id ? 'success' : 'info'}>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="success" onClick={handleCreate} disabled={isSubmitting} loading={isSubmitting} className="flex-1">
              Подтвердить
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={showQcRawModal} onOpenChange={setShowQcRawModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Добавить QC сырья</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={showQcProductModal} onOpenChange={setShowQcProductModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Добавить QC продукта</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {qcTestTypes.map(test => (
      ...
      <Dialog open={showProcessRawModal} onOpenChange={setShowProcessRawModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Добавить процессинг сырья</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-64 overflow-y-auto">
      ...
      <Dialog open={showProcessProductModal} onOpenChange={setShowProcessProductModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Добавить процессинг (до розлива)</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-64 overflow-y-auto">
      ...
      <Dialog open={showPostProcessProductModal} onOpenChange={setShowPostProcessProductModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Добавить постпроцессинг (после розлива)</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-64 overflow-y-auto">
            ...
            {processMethods.filter((m: any) => m.stage === 'post_filling').length === 0 && (
              <p className="text-muted-foreground text-sm">Нет доступных методов постпроцессинга</p>
            )}
          </div>
        </DialogContent>
      </Dialog>
        ...
        <table className="w-full">
          <thead className="bg-muted">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Срок</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase">Осталось</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Тип</th>
              ...
              return (
                <tr key={req.request_id} className={`hover:bg-muted/50 ${isOverdue ? 'bg-red-50' : isUrgent24h ? 'bg-red-50' : isUrgent72h ? 'bg-amber-50' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <Badge variant={(req as any).type === 'MTO' ? 'info' : 'secondary'}>
                      {(req as any).type || 'MTS'}
                    </Badge>
                  </td>
