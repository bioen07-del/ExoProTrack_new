...
export default function RequestDetail() {
  ...
  async function handleReserve() {
    ...
    if (reserveVolume > availableVolume) {
      const confirmed = await showConfirm(
        `Доступно только ${availableVolume.toFixed(1)} мл. Зарезервировать доступный объем и создать новую заявку на остаток?`
      );
      if (!confirmed) {
        return;
      }
  ...
  async function handleCompleteDraftRequest(line: RequestLine) {
    if (!draftSourceType) {
      showError('Выберите источник сырья');
      return;
    }
  ...
  async function handleClosePartialLine(line: RequestLine) {
    const confirmed = await showConfirm(
      `Закрыть строку как выполненную с ${(line as any).qty_fulfilled} из ${line.qty_units} шт?`
    );
    if (!confirmed) return;
    try {
      await supabase.from('request_line')
  ...
  async function handleCreateRemainderRequest(line: RequestLine) {
    ...
    const confirmed = await showConfirm(`Создать новую заявку на ${remainder} шт?`);
    if (!confirmed) return;
    ...
    try {
      ...
      showSuccess(`Создана заявка ${newReqId} на ${remainder} шт`);
      loadData();
    } catch (err: any) {
      showError('Ошибка', err.message);
    }
  ...
  async function createPackLot(line: RequestLine) {
    ...
    if (lineReservations.length === 0) {
      showError('Нет активных резервов для этой строки');
      return;
    }
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64 text-foreground">Загрузка...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-bold text-foreground font-mono">{request.request_id}</h1>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-semibold text-foreground">Статус производства CM</h3>
                ...
                return (
                  <React.Fragment key={step.key}>
                    <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      isCurrent ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' :
                    ...
                    {idx < PRODUCTION_STEPS.length - 1 && (
                      <div className={`flex-1 h-0.5 ${isCompleted ? 'bg-green-400' : 'bg-border'}`} />
                    )}
                  </React.Fragment>
                );
              })}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <h4 className="font-medium text-muted-foreground mb-3">QC сырья</h4>
        ...
        <Card>
          <CardContent className="p-4">
            <h4 className="font-medium text-muted-foreground mb-3">QC продукта</h4>
            <div className="flex flex-wrap gap-2">
      ...
      {request.status === 'New' && lines.length > 0 && !lines[0].source_type && (
        <div className="bg-amber-50 dark:bg-amber-950/30 border-2 border-amber-300 dark:border-amber-700 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-amber-800 dark:text-amber-400">Завершите оформление заявки</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Продукт</label>
              <p className="px-3 py-2 bg-card rounded border border-border text-foreground">{lines[0].finished_product_code}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Количество (шт)</label>
              <Input
                type="number"
                min={1}
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-foreground mb-2">Источник сырья</label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4"
                />
                <span className="text-foreground">Со склада (готовое)</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4"
                />
                <span className="text-foreground">Новое производство (MTO)</span>
              </label>
            </div>
          </div>
          ...
          {draftSourceType === 'FromStock' && (
            <div className="mb-4">
              <label className="block text-sm font-medium text-foreground mb-1">Выберите партию сырья</label>
              <select
                value={draftCmLotId}
        ...
        <CardContent>
          <div className="space-y-4">
            {lines.map((line) => {
              ...
              return (
                <div key={line.request_line_id} className="p-4 bg-muted rounded-lg">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-foreground">{line.finished_product_code}</p>
                        <Button
                          variant="ghost"
                          size="sm"
                      ...
                      <p className="text-sm text-muted-foreground">
                        ...
                        <Badge variant={isLineMTO ? 'info' : 'muted'} className="ml-1">
                  ...
                  {lineReservations.length > 0 && (
                    <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-950/30 rounded text-sm">
                      <p className="font-medium text-blue-800 dark:text-blue-400">Резервы:</p>
                  ...
                  {(line as any).qty_fulfilled > 0 && (line as any).qty_fulfilled < line.qty_units && (
                    <div className="mt-2 p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-700 rounded text-sm">
                      <p className="font-medium text-amber-800 dark:text-amber-400 flex items-center gap-2">
                        <AlertTriangle size={16} />
                        Частичное исполнение: {(line as any).qty_fulfilled} из {line.qty_units} шт
                      </p>
                      <p className="text-amber-700 dark:text-amber-400 mt-1">Не хватает: {line.qty_units - (line as any).qty_fulfilled} шт</p>
                  ...
                  {linePacks.length > 0 && (
                    <div className="mt-2 p-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-700 rounded-lg">
                      <p className="font-medium text-emerald-800 dark:text-emerald-400 mb-2">Продукт создан:</p>
                      {linePacks.map(p => (
                        <div key={p.pack_lot_id} className="flex justify-between items-center bg-card p-2 rounded">
                          <div>
                            <Link to={`/packlot/${p.pack_lot_id}`} className="font-mono font-bold text-emerald-700 dark:text-emerald-400 hover:underline">
                              {p.pack_lot_id}
                            </Link>
                            <p className="text-xs text-muted-foreground">Продолжить работу на странице продукта →</p>
                          </div>
                          <div className="flex items-center gap-2">
      ...
      {/* Reserve Modal */}
      <Dialog open={showReserveModal && !!selectedLine} onOpenChange={(open) => {
        if (!open) {
          setShowReserveModal(false);
          setSelectedLine(null);
          setSelectedCmLot('');
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Резервирование сырья</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Выберите CM Lot (QA одобрено)</label>
              <select
                value={selectedCmLot}
            ...
            {selectedCmLot && (
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Объем резерва (мл)</label>
                <Input
                  type="number"
                  step={0.1}
                  value={reserveVolume}
